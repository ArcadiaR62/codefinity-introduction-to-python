...
for item in inventory:
    if inventory.get(item)[0] < 30:
        print(f"{item} need restocking.")
    elif inventory.get(item)[0] >100:
        print(f"{item} should be sold at the discounted price of {inventory.get(item)[2]}.")
    else:
        print(f"{item} should be sold at the regular price of {inventory.get(item)[1]}.")